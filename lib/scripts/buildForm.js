document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    console.error("Form or inputs missing, aborting event listeners");
    return;
  }
  ...
  const validateName = (name) => {
    ...
    if (!/^[a-zA-Z\s]{2,}$/.test(name))
      return "El nombre debe contener solo letras y espacios (mínimo 2 caracteres).";
    return "";
  };
  ...
  const validateSelect = (value, fieldName) => {
    if (!value || value === "") return `Elegí un ${fieldName}.`;
    return "";
  };
  ...
  const validateForm = () => {
    ...
    const ramErrorMsg = validateSelect(ramInput.value, "módulo de RAM");
    if (ramErrorMsg) {
  ...
  ramInput.addEventListener("change", () => {
    updateError(
      ramInput,
      ramError,
      validateSelect(ramInput.value, "módulo de RAM")
    );
  });
