                ...
                <?php foreach ($tags as $tag) : ?>
                    <tr class="w-full flex flex-col items-center justify-between bg-[#D9D9D913] p-4 rounded-sm md:flex-row">
                        <td class="flex-1">
                            ...
                            <form id="update-form-<?= $tag->getId() ?>" action="actions/tags/update-tag.php" method="POST" class="hidden">
                                <input type="hidden" name="id" value="<?= $tag->getId() ?>">
                                <input type="text" name="name" value="<?= $tag->getName() ?>" required class="px-2 py-1 border border-gray-300 rounded-sm">
